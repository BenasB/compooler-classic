...
public static class GroupDateTime
{
    ...
    /// <param name="searchStart">Group's local time to start searching the next start datetime from</param>
    ...
    public static DateTime GetNextStartDateTime(DateTime searchStart, int days, TimeOnly startTime)
    {
        ...
        for (var i = 0; i < 7; i++)
        {
            // Check the days starting from the searchStart's day of week
            var day = (DayOfWeek)(((int)searchStart.DayOfWeek + i) % 7);

            if ((rotatedDays & (1 << (int)day)) == 0)
                continue; // Skip days on which the group does not commute

            int daysToAdd = ((int)day - (int)searchStart.DayOfWeek + 7) % 7;

            var nextStartDate = searchStart.AddDays(daysToAdd);
            ...
            if (daysToAdd == 0)
            {
                ...
                if (nextStartDateTime <= searchStart)
                {
                    var dayAfterLatest = searchStart.AddDays(1);
